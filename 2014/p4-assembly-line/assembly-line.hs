...
getPairs 0 = return []
getPairs n = do
    [t1, t2] <- map read . words <$> getLine :: IO [Int]
    ps       <- getPairs (n - 1)
    return ((t1, t2) : ps)
...
getQuads 0 = return []
getQuads n = do
    [m1, m2, c1, c2] <- map read . words <$> getLine :: IO [Int]
    ps               <- getQuads (n - 1)
...
dynprog ((t1, t2) : ts) ((m1, m2, c1, c2) : qs)
    | not cross1 && not cross2 = (S (1 : p1) (t1 + m1'),
                                  S (2 : p2) (t2 + m2'))
    | not cross1 &&     cross2 = (S (1 : p1) (t1 + m1'),
                                  S (2 : p1) (t2 + c2'))
    |     cross1 && not cross2 = (S (1 : p2) (t1 + c1'),
                                  S (2 : p2) (t2 + m2'))
    |     cross1 &&     cross2 = (S (1 : p2) (t1 + c1'),
                                  S (2 : p1) (t2 + c2'))
  where
    m1'    = m1 + tau1
    m2'    = m2 + tau2
